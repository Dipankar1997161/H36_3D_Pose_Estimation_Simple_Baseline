...
tf.compat.v1.flags.DEFINE_float("dropout", 1, "Dropout keep probability. 1 means no dropout")
tf.compat.v1.flags.DEFINE_integer("batch_size", 128, "Batch size to use during training")
tf.compat.v1.flags.DEFINE_integer("epochs", 150, "How many epochs we should train for")
tf.compat.v1.flags.DEFINE_boolean("camera_frame", False, "Convert 3d poses to camera coordinates")
tf.compat.v1.flags.DEFINE_boolean("max_norm", False, "Apply maxnorm constraint to the weights")
tf.compat.v1.flags.DEFINE_boolean("batch_norm", False, "Use batch_normalization")

# Data loading
tf.compat.v1.flags.DEFINE_boolean("predict_14", False, "predict 14 joints")
tf.compat.v1.flags.DEFINE_string("action","All", "The action to train on. 'All' means all the actions")

# Architecture
tf.compat.v1.flags.DEFINE_integer("linear_size", 1024, "Size of each model layer.")
tf.compat.v1.flags.DEFINE_integer("num_layers", 2, "Number of layers in the model.")
tf.compat.v1.flags.DEFINE_boolean("residual", False, "Whether to add a residual connection every 2 layers")

# Evaluation
tf.compat.v1.flags.DEFINE_boolean("procrustes", False, "Apply procrustes analysis at test time")
tf.compat.v1.flags.DEFINE_boolean("evaluateActionWise", False, "The dataset to use either h36m or heva")

# Directories
tf.compat.v1.flags.DEFINE_string("cameras_path","/3D_Pose_Estimation_With_Simple_Baseline/data/h36m/metadata.xml", "File with h36m metadata, including cameras")
tf.compat.v1.flags.DEFINE_string("data_dir","/3D_Pose_Estimation_With_Simple_Baseline/data/h36m/", "Data directory")
tf.compat.v1.flags.DEFINE_string("train_dir", "/3D_Pose_Estimation_With_Simple_Baseline/experiments", "Training directory.")

# Train or load
tf.compat.v1.flags.DEFINE_boolean("sample", False, "Set to True for sampling.")
tf.compat.v1.flags.DEFINE_boolean("use_cpu", False, "Whether to use the CPU")
tf.compat.v1.flags.DEFINE_integer("load", 0, "Try to load a previous checkpoint.")

# Misc
tf.compat.v1.flags.DEFINE_boolean("use_fp16", False, "Train using fp16 instead of fp32.")
...
def train():
  ...
  device_count = {"GPU": 0} if FLAGS.use_cpu else {"GPU": 2}
  print(device_count)
...
def sample():
  ...
  with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto( device_count = device_count )) as sess:
    # === Create the model ===
  ...
  plt.savefig('/3D_Pose_Estimation_With_Simple_Baseline/plots/viz_epoch'+ str(FLAGS.epochs) +'.png', dpi=100)
  plt.show()
